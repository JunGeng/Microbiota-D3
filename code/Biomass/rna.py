...
def _get_mRNA_fractions(df, ):
    ...
    mean_abundance = df.copy()
    mean_abundance['abundances'] = 1
...
def _process_record(path_to_genbank, identifier):
            ...
            for i, element in enumerate(record.features):
                ...
                if element.type == 'tRNA':
                    ...
                    try:
                        ...
                        if before.type == 'gene':
                            try:
                                tRNA_locus.append(before.qualifiers['locus_tag'][0])
                            except:
                                raise Exception("Can't fetch tRNA locus_tag!")
                        else:
                ...
                if element.type == 'rRNA':
                    ...
                    try:
                        ...
                        if before.type == 'gene':
                            try:
                                rRNA_locus.append(before.qualifiers['locus_tag'][0])
                            except:
                                raise Exception("Can't fetch rRNA locus_tag!")
                        else:
    ...
    mRNA_dict = _get_mRNA_fractions(mRNA_df)
...
def _convert_to_mmolgDW(RNA_coefficients, RNA_RATIO, CELL_WEIGHT):
    ...
    rna_base_to_bigg = {'A': 'atp_c', 'U': 'utp_c',
                        'C': 'ctp_c', 'G': 'gtp_c'}
    rna_base_to_bigg_formula_weight = {'A': 503.149263, 'U': 480.109323,
                                       'C': 479.124563, 'G': 519.148663}
    ...
    for letter in BASES:
        ...
        formula_weight = rna_base_to_bigg_formula_weight.get(letter)
        mol_weight = formula_weight - DIPHOSPHATE_WEIGHT
...
def generate_coefficients(path_to_genbank,
    ...
    rRNA_dict, tRNA_dict, mRNA_dict = _process_record(path_to_genbank, identifier)
...
if __name__ == '__main__':
    ...
    rna_weight_fraction = 0.205
    genbank = '../archive/Ecoli_K12_MG1655.gbff'
    transcriptomic = '../archive/transcriptomic.csv'
    model = '../archive/iML1515.json'
    # rna_coefficients = BOFdat.step1.generate_rna_coefficients(genbank,model,transcriptomic,RNA_WEIGHT_FRACTION=rna_weight_fraction)
    # print(rna_coefficients)
    #  {<Metabolite atp_c at 0x7fde8de52a90>: -0.1556843351415502,
    #  <Metabolite utp_c at 0x7fde8de56150>: -0.15379576979105725,
    #  <Metabolite ctp_c at 0x7fde8de4ca10>: -0.1671601704512458,
    #  <Metabolite gtp_c at 0x7fde8de5fdd0>: -0.16306640467110822,
    #  <Metabolite ppi_c at 0x7fde8de573d0>: 0.6397066800549615}

    RNA_biomass_ratios,RNA_coefficients = generate_coefficients(genbank,
                                               RNA_WEIGHT_FRACTION=rna_weight_fraction,
                                               rRNA_WEIGHT_FRACTION=0.9,
                                               tRNA_WEIGHT_FRACTION=0.05,
                                               mRNA_WEIGHT_FRACTION=0.05,
                                               identifier='locus_tag')
    # biomass_coefficients = biomass_coefficients * DNA_WEIGHT_FRACTION
    print(RNA_biomass_ratios,RNA_coefficients)
